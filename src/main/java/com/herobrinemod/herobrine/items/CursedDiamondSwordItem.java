...
public class CursedDiamondSwordItem extends SwordItem {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, @NotNull List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.herobrine.cursed_diamond_sword.kills").append(ScreenTexts.SPACE).append(String.valueOf(this.kills)));
        super.appendTooltip(stack, world, tooltip, context);
    }

    @Override
    public TypedActionResult<ItemStack> use(@NotNull World world, @NotNull PlayerEntity user, Hand hand) {
        if((user.isCreative() || this.kills >= ConfigHandler.getHerobrineConfig().readInt("CursedDiamondSwordChargeCost")) && !user.isSneaking()) {
            ...
            if (!user.isCreative()) {
                ...
                if(!world.isClient) {
                    this.kills = kills - ConfigHandler.getHerobrineConfig().readInt("CursedDiamondSwordChargeCost");
                }
            }
