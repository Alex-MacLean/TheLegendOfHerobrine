...
public class InfectedLlamaEntity extends LlamaEntity {
    ...
    public boolean attackEntityFrom(@NotNull DamageSource source, float amount) {
        if (source.getImmediateSource() instanceof HolyWaterEntity) {
            ...
            llamaEntity.setLocationAndAngles(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
    ...
    private void spit(@NotNull LivingEntity target) {
        ...
        double d1 = target.posY - llamaSpitEntity.posY;
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            IForgeBlockState state = this.world.getBlockState(pos);
            if (!state.isAir(world, pos) && !this.isSilent()) {
